...
describe(`testing scheduling-srv ${testSuffix}: gRPC`, () => {
  ...
  beforeAll(async function (): Promise<any> {
    ...
    const offset = await jobEvents.$offset(BigInt(-1));

    await grpcSchedulingSrv.delete({ collection: true });

    if (toDelete! > 0) {
      await jobEvents.$wait(BigInt(Number(offset) + toDelete! - 1));
    }
  ...
  describe(`create a one-time job ${testSuffix}`, { timeout: 30000 }, () => {
    it(`should create a new job and execute it immediately ${testSuffix}`, async () => {
      ...
      await jobEvents.on('jobDone', async (job) => {
      ...
      await jobEvents.$wait(BigInt(Number(offset) + 1));
    ...
    it(`should create a new job and execute it at a scheduled time ${testSuffix}`, async () => {
      ...
      await jobEvents.$wait(BigInt(Number(offset) + 1)); // jobsCreated, queuedJob (jobDone is sent from test)
  ...
  describe(`should create a recurring job ${testSuffix}`, { timeout: 8000 }, function (): void {
    it(`should create a recurring job and delete it after some executions ${testSuffix}`, async () => {
      ...
      // wait for 3 'jobDone'
      await jobEvents.$wait(BigInt(Number(offset) + 3));
